...
export function useOperacoesStats(userId?: string, dateRange?: { from?: Date; to?: Date }) {
  return useQuery({
    queryKey: ['operacoes-stats', userId, dateRange?.from?.toISOString(), dateRange?.to?.toISOString()],
...
export function useAllHistorico(userId?: string) {
  return useQuery({
    queryKey: ['all-historico', userId],
    queryFn: async () => {
      let query = supabase
        .from('movimentacoes_historico')
        .select(`
          *,
          usuario:profiles!usuario_id(nome, avatar_url),
          operacao:operacoes!operacao_id(numero_funil, empresa:empresas(nome))
        `)
        .order('data_hora', { ascending: false })
        .limit(50);
